this.xxu_armor <- this.inherit("scripts/items/item", {
	m = {},
	function create()
	{
		this.item.create();
		this.m.ID = "misc.xxu_armor";
		this.m.Name = "Enhancement Scroll (Armor)";
		this.m.Description = "Enhances the durability of the equipped [color=#a13600]Named Armor[/color] of the current owner. The enhancement has a 50% chance of success, increasing the armor\'s durability by 1~10% of its current value. If the enhancement fails, there is a 10% chance that the armor will be destroyed. Only named items can be enhanced.";
	...
	function onUse( _actor, _item = null )
	{
		...
		if (mitem != null && mitem.isItemType(this.Const.Items.ItemType.Named) && mitem.isItemType(this.Const.Items.ItemType.Armor))
		{
