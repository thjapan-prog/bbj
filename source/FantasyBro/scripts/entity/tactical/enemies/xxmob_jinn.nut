this.xxmob_jinn <- this.inherit("scripts/entity/tactical/actor", {
	m = {
		Tail = null
	},
	...
	function create()
	{
		this.m.Name = this.Const.Strings.FT.M49;
	...
	function onTurnStart()
	{
		if (this.m.Tail != null && !this.m.Tail.isNull() && this.m.Tail.isAlive())
		{
			if (this.actor.getHitpoints() != this.m.Tail.getHitpoints())
			{
				local hp1 = this.actor.getHitpoints();
				local hp2 = this.m.Tail.getHitpoints();
				this.actor.setHitpoints(this.Math.min(hp1, hp2));
				this.m.Tail.setHitpoints(this.Math.min(hp1, hp2));
	...
	function onInit()
	{
		...
		if (this.m.Tail == null)
		{
			...
			if (spawnTile != null)
			{
				this.m.Tail = this.WeakTableRef(this.Tactical.spawnEntity("scripts/entity/tactical/enemies/xxmob_jinn_tail", spawnTile.Coords.X, spawnTile.Coords.Y, this.getID()));
