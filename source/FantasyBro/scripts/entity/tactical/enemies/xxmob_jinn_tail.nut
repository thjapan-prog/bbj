this.xxmob_jinn_tail <- this.inherit("scripts/entity/tactical/actor", {
	m = {
		Body = null
	},
	...
	function create()
	{
		this.m.Name = this.Const.Strings.FT.M50;
		...
		this.m.SoundVolume[this.Const.Sound.ActorEvent.Attack] = 1.0;
	...
	{
		if (this.m.Body != null && !this.m.Body.isNull() && this.m.Body.isAlive())
		{
			if (this.actor.getHitpoints() != this.m.Body.getHitpoints())
			{
				local hp1 = this.actor.getHitpoints();
				local hp2 = this.m.Body.getHitpoints();
				this.actor.setHitpoints(this.Math.min(hp1, hp2));
				this.m.Body.setHitpoints(this.Math.min(hp1, hp2));
				if (this.getTile().IsVisibleForPlayer)
				{
					this.Tactical.spawnIconEffect("status_effect_01", this.getTile(), this.Const.Tactical.Settings.SkillIconOffsetX, this.Const.Tactical.Settings.SkillIconOffsetY, this.Const.Tactical.Settings.SkillIconScale, this.Const.Tactical.Settings.SkillIconFadeInDuration, this.Const.Tactical.Settings.SkillIconStayDuration, this.Const.Tactical.Settings.SkillIconFadeOutDuration, this.Const.Tactical.Settings.SkillIconMovement);
				}
				this.Sound.play("sounds/enemies/unhold_regenerate_01.wav", 1.5, this.getTile().Pos);
				this.actor.updateOverlay();
				this.m.Body.updateOverlay();
			}
		}
	...
	function onInit()
	{
		...
		body.setBrush("bust_jinn_1");
		local injury = this.addSprite("injury");
		injury.setBrush("bust_alp_01_injured");
		injury.Scale = 0.6;
		injury.Color = this.createColor("#ff0000");
		...
		this.m.Skills.add(this.new("scripts/skills/actives/ai_jinn_skill"));
