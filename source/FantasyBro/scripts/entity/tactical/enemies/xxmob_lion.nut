this.xxmob_lion <- this.inherit("scripts/entity/tactical/actor", {
	...
	function create()
	{
		this.m.Name = this.Const.Strings.FT.M54;
	...
	function onDeath( _killer, _skill, _tile, _fatalityType )
	{
		...
		foreach( a in gg )
		{
			...
			if (a.isAlive() && a.getFlags().has("lion") && !a.isPlayerControlled())
			{
				a.getBaseProperties().MeleeSkill += 3;
				a.getBaseProperties().RangedSkill += 3;
				a.getBaseProperties().Bravery += 3;
				a.getBaseProperties().MeleeDefense += 1;
				a.getBaseProperties().Hitpoints += 30;
				a.getSkills().add(this.new("scripts/skills/actives/ai_fakebuff"));
				if (a.getMoraleState() < this.Const.MoraleState.Steady && this.Math.rand(1, 100) <= 35)
				{
