this.legend_inventor_prosthetic_leg <- this.inherit("scripts/events/event", {
	m = {
		Inventor = null,
		Nofoot = null
	},
	function create()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				local maimed_foot_bye = this.new("scripts/skills/injury_permanent/broken_knee_injury");
				_event.m.Nofoot.getSkills().removeByID("injury.broken_knee");
				this.List.push({
						id = 10,
