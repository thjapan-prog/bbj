this.xxmob_spiderdemon <- this.inherit("scripts/entity/tactical/actor", {
	m = {
		Size = 1.0,
		DistortTargetA = null,
	...
	function create()
	{
		this.m.Name = this.Const.Strings.FT.M67;
	...
	function onDeath( _killer, _skill, _tile, _fatalityType )
	{
		if (_tile != null)
		{
			...
			local xxcolor = this.createColor("#8e63a8");
	...
	function onInit()
	{
		...
		legs_back.setBrush("bust_xxspider_legs_back");
		local body = this.addSprite("body");
		body.setBrush("bust_xxspider_body_0" + this.Math.rand(1, 2));
		...
		legs_front.setBrush("bust_xxspider_legs_front");
		...
		head.setBrush("bust_xxspider_head_01");
		...
		wtt.m.RangeMax = 1;
		wtt.m.RangeIdeal = 1;
		wtt.m.RegularDamage = 70;
		wtt.m.RegularDamageMax = 120;
		wtt.m.ArmorDamageMult = 1;
		...
		this.m.Skills.add(this.new("scripts/skills/actives/ai_spider_passive"));
		local skilla = this.new("scripts/skills/actives/ai_monsterbite");
		skilla.m.SoundVolume = 1.5;
		skilla.m.Overlay = "active_115";
		skilla.m.ActionPointCost = 6;
		skilla.m.SoundOnUse = [
		...
		skilla.m.MaxRange = 1;
		skilla.m.DirectDamageMult = 0.4;
		this.m.Skills.add(skilla);
		skilla = this.new("scripts/skills/actives/ai_poison_passive");
		skilla.setDamage(5);
		this.m.Skills.add(skilla);
	}
