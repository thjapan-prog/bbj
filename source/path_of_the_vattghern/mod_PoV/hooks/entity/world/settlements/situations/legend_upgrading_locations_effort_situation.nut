::TLW.HooksMod.hook("scripts/entity/world/settlements/situations/legend_upgrading_locations_effort_situation", function(q) 
{
	q.getTooltip = @(__original) function()
	{
		...
		if(::TLW.PovSituation)
		{
			ret.push({
				...
				text = "Building Materials Value increased by [color=" + this.Const.UI.Color.PositiveValue + "]100%[/color]"
			});
			ret.push({
				id = 8,
				type = "text",
				icon = "ui/icons/pov_situation_wood.png",
				text = "Available Building Materials reduced by [color=" + this.Const.UI.Color.NegativeValue + "]25%[/color]"
			});
			...
			ret.push({
				...
				text = "After some time, the settlement will be able to construct an additional location"
			});
		}
		return ret;
	}
