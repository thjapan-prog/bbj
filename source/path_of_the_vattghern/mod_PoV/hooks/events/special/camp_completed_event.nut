::TLW.HooksMod.hook("scripts/events/events/special/camp_completed_event", function(q)
{
	q.create = @(__original) function ()
	{
		//__original(); // I need to overwrite
		this.m.ID = "event.camp_completed";
		this.m.Title = "Camp Report";
		this.m.IsSpecial = true;
		this.m.Screens.push({
			ID = "A",
			Text = "[img]gfx/ui/events/pov_camp_report.png[/img][color=" + this.Const.UI.Color.povEvent + "]Afer a well deserved rest, you receive a detailed report of the company\'s activities.[/color]",
			Image = "",
			Banner = "",
			List = [],
			Characters = [],
