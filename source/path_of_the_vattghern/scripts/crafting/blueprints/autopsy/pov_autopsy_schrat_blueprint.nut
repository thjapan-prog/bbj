this.pov_autopsy_schrat_blueprint <- this.inherit("scripts/crafting/blueprint", {
	m = {},
	function create()
	{
		this.blueprint.create();
		this.m.ID = "blueprint.pov_autopsy_schrat";
		this.m.Type = this.Const.Items.ItemType.Usable;
		this.m.PreviewCraftable = this.new("scripts/items/autopsy/pov_autopsy_schrat_item");
		this.m.Cost = 115;
		local ingredients = [
			{
	...
	function onCraft( _stash )
	{
		...
		if (this.Math.rand(1, 100) <= 30)
			{
				item = this.new("scripts/items/misc/ancient_wood_item");
				_stash.add(item);
				if (this.World.Assets.isCamping()){this.World.Camp.getBuildingByID(this.Const.World.CampBuildings.Crafting).addCraftedItem(item);}
			}
		if (this.Math.rand(1, 100) <= 30)
