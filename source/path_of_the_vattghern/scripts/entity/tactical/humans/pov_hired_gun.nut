this.pov_hired_gun <- this.inherit("scripts/entity/tactical/human", {
	m = {
		IsLow = false
	},
	function create()
	{
		this.m.Type = this.Const.EntityType.PovHiredGun;
		this.m.BloodType = this.Const.BloodType.Red;
		this.m.XP = this.Const.Tactical.Actor.PovHiredGun.XP;
		this.human.create();
	...
	function onInit()
	{
		...
		b.setValues(this.Const.Tactical.Actor.PovHiredGun);
	...
	function assignRandomEquipment()
	{
		...
		{
			this.m.Items.addToBag(this.new("scripts/items/weapons/scimitar"));
		}
		else if (r == 3)
		{
		...
		local equipmentSets = [
			{ armor = "pov_hired_gun_armor", helmet = "pov_hired_gun_helmet" }
		];
