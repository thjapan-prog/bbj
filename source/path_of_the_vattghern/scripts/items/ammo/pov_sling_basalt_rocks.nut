this.pov_sling_basalt_rocks <- this.inherit("scripts/items/ammo/ammo", {
	m = {},
	function create()
	{
		this.ammo.create();
		this.m.ID = "ammo.pov_sling_basalt_rocks";
		this.m.Name = "Basalt Rocks";
		this.m.Description = "A pouch of crude and heavy basalt rocks, meant to be used as sling ammunition. They can cause destructive damage due to their weight, but for the same reason the require much more effort and skill to effectively use, and reduce effective range.\n\n When used, they consume and are refilled by ammunition normally, but if they run out, the slinger can still use normal stones";
		this.m.Icon = "ammo/pov_pouch_basalt.png";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "Ranged attacks deal [color=" + this.Const.UI.Color.PositiveValue + "]20%[/color] more damage, and are also [color=" + this.Const.UI.Color.PositiveValue + "]20%[/color] more effective against armor."
		});
		...
		result.push({
			...
			text = "Ranged attack range decreased by [color=" + this.Const.UI.Color.NegativeValue + "]2[/color]."
		});
		...
		result.push({
			...
			type = "text",
			icon = "ui/icons/fatigue.png",
			text = "Ranged attacks require [color=" + this.Const.UI.Color.NegativeValue + "]50%[/color] more fatigue to use."
		});
	...
	function onAnySkillUsed( _skill, _targetEntity, _properties )
	{
		...
		if (_skill.isAttack() && item != null && item.isItemType(this.Const.Items.ItemType.Weapon) && item.isWeaponType(this.Const.Items.WeaponType.Sling) && this.m.Ammo != 0)
		{
			...
			//_properties.DamageDirectMult *= 0.80;
		}
	}
	...
	function onUpdateProperties(_properties)
	{
		...
		{			
			...
			foreach (skill in skills)
			{
				...
				{
					skill.m.FatigueCost *= 1.50;
					//skill.m.HitChanceBonus -= 4;
				}
			}
		} 
	}
