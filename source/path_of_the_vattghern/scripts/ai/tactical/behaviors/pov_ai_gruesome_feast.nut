this.pov_ai_gruesome_feast <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		TargetTile = null,
		PossibleSkills = [
			"actives.pov_gruesome_feast"
		],
		Skill = null,
		IsTravelling = false,
		IsWaiting = false
	...
	function create()
	{
		this.m.ID = this.Const.AI.Behavior.ID.PovGruesomeFeast;
		this.m.Order = this.Const.AI.Behavior.Order.PovGruesomeFeast;
	...
	function onEvaluate( _entity )
	{
		...
		if (_entity.getHitpointsPct() >= 0.75)
		{
			return this.Const.AI.Behavior.Score.Zero;
		}
		...
		else
		{
			foreach( c in corpses )
			{
				...
				if (dist > 1)
				{
					...
					{
						if (!ally.isTurnDone() && ally.getTile().getDistanceTo(c) == 1 && ally.getType() == this.Const.EntityType.Ghoul)
						{
							letComradeGo = true;
							break;
						}
