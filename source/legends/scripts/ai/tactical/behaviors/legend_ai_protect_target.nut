this.legend_ai_protect_target <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		Target = null,
		TargetTile = null,
	...
	function selectBestTargetTile( _entity )
	{
		...
		local importantAllyTile = target.getTile();
		local dirs = [
			0,
			0,
			0,
					...
					for( local j = 0; j != 6; j = ++j )
					{
						...
						else
						{
							...
							else
							{
								...
								{
									score = score * target.getCurrentProperties().TargetAttractionMult + 2;
								}
							}
						}
					}
